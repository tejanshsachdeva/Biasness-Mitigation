...
import numpy as np
BASE_DIR = os.getenv("BASE_DIR", r"C:\AllMyCodes\Major_Project_Code\Fine_Tuned Model\ft_llama")  
...
GRAD_ACCUMULATION = int(os.getenv("GRAD_ACCUMULATION", 4))
...
class ModelTrainer:
    ...
    def train(self, merge_weights: bool = False):
        ...
        print("\nTraining complete! Model saved in:")
        print(f"- LoRA weights: {LORA_DIR}")
        if merge_weights:
            print(f"- Merged model: {MERGED_DIR}")
        print(f"- Training logs: {LOGS_DIR}")
        print(f"- Configuration: {os.path.join(OUTPUT_DIR, 'training_config.json')}")
...
def main():
    try:
        ...
        trainer.train(merge_weights=True)
